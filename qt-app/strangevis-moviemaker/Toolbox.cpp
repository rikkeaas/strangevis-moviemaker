...
Toolbox::Toolbox(const QString & title, const int animationDuration, QWidget* parent)
    : QWidget(parent), animationDuration(animationDuration)
{
    ...
    toggleButton->setStyleSheet("QToolButton {border: 5px solid green; color:red;}");
    ...
    int row = 0;
    mainLayout->addWidget(toggleButton, row, 0, 1, 3, Qt::AlignLeft);
    mainLayout->addWidget(headerLine, row++, 2, 1, 3);
    mainLayout->addWidget(contentArea, row, 0, 1, 3);
    setLayout(mainLayout);

    connect(toggleButton, &QToolButton::toggled, this, &Toolbox::toggle);
}


void Toolbox::toggle(bool collapsed) {
    toggleButton->setArrowType(collapsed ? Qt::ArrowType::DownArrow : Qt::ArrowType::LeftArrow);
    toggleAnimation->setDirection(collapsed ? QAbstractAnimation::Forward : QAbstractAnimation::Backward);
    toggleAnimation->start();
}


void Toolbox::setContentLayout(QLayout & contentLayout)
{
